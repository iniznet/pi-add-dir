...
- **fix:** Saved dirs are stored in `$XDG_CONFIG_HOME/pi-add-dir/dirs/<hash>.json` (Unix), `$APPDATA/pi-add-dir/dirs/<hash>.json` (Windows), or `~/.config/pi-add-dir/dirs/<hash>.json` (fallback)
