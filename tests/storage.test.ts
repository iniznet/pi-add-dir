...
// ---------------------------------------------------------------------------
// Setup / Teardown — redirect storage to temp dir
// ---------------------------------------------------------------------------

// Override the config base path so saved dirs are stored
// in our temp dir instead of the real ~/.pi/agent/
...
describe("loadSavedDirs", () => {
  ...
  it("validates required fields", () => {
    const cwd = "/my/project";
    const dirsDir = path.join(tmpHomeDir, ".pi", "agent", "dirs");
...
describe("saveSavedDirs", () => {
  ...
  it("creates config directory if it doesn't exist", () => {
    const cwd = "/my/project";
    // tmpHomeDir is empty — .pi/agent/dirs/ shouldn't exist yet
    const dirs = [makeSavedDir("/lib/a")];
    saveSavedDirs(cwd, dirs);

    const configDirs = path.join(tmpHomeDir, ".pi", "agent", "dirs");
